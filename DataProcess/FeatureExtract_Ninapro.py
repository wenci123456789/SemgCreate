...
class Extractor(Matloader):
    ...
    def extract(self, data=None, method=None):
        ...
        for eachmethod in method:
            print(data.shape)
            print((data.shape[0] - self.__windowsize + 1) // self.__step)
    ...
    def split_dataset(self, data, division=1, train_ratio=0.7, if_val=True):
    ...
    def choose_save_actions(self, stimulus=None, savedir="../Data/featureset1", with_additional_loads=True,
                            train_ratio=0.7, if_val=True, extract_method="rms", keep_rest=False):
        ...
        self.save(feature_train, savedir=savedir, suffix=f"{extract_method}_train")
        self.save(feature_test, savedir=savedir, suffix=f"{extract_method}_test")

        self.save(total_glove_train, savedir=savedir, suffix="glove_train")
        self.save(total_glove_test, savedir=savedir, suffix="glove_test")

        if if_val:
            self.save(feature_val, savedir=savedir, suffix=f"{extract_method}_val")
            self.save(total_glove_val, savedir=savedir, suffix="glove_val")
        print("[*]Written!")
...
if __name__ == "__main__":
    ...
    # numlist.remove(26-1)
    # numlist.remove(40-1)
    data_dir_list = [f"E:/semg/dataset/nianpro/db2/DB2_s1//S1_E1_A1.mat" ]#f"../Data/rawdata/S{x + 1}_E1_A1.mat" for x in range(0,40)
    # print(data_dir_list)
    # exit(0)
    extractor_list = []
    ...
    for each_extractor in extractor_list:
        each_extractor.choose_save_actions(train_ratio=0.7, if_val=False, stimulus=[18, 19, 22, 25, 27, 37],
