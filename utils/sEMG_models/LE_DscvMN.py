...
from model.DscvLstm import DscvLSTM
from utils.BaseModels.ConvLSTM.ConvLSTM import ConvLSTM


class LE_DscvMN(nn.Module):
    """
    Parameters:
    ...
    def __init__(self, input_size, input_dim, output_dim, hidden_dim, kernel_size, num_layers, outpoint,
        ...
        super(LE_DscvMN, self).__init__()

        # Make sure that both `kernel_size` and `hidden_dim` are lists having len == num_layers
        self.name = "LE_DesvMN"
        self.DesvMN = DscvLSTM(input_size, input_dim, output_dim, hidden_dim, kernel_size, num_layers, outpoint,
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        output, hidden_state = self.DesvMN(input_tensor, hidden_state)
...
if __name__ == '__main__':# 测试PSA模块
    hidden = None
    input = torch.randn(16, 200, 12, 1).cuda()# 创建一个随机输入
    model = LE_DscvMN((12, 200), 1, 1, [64, 32, 10], (3, 3), 3, 20, batch_first=True, bias=True,
                      return_all_layers=False, withCBAM=False).cuda()
    output, hidden = model(input, hidden)  # RNN
