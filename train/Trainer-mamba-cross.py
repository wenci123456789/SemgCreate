...
for pretrain in pretrains:
    # Params
    parser = argparse.ArgumentParser(description='PyTorch sEMG-mamba')
    # dataset
    ...
    if "S0" in subject:
        subject.remove("S0")
        subject = ["S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8", "S9", "S10", "S11", "S12", "S13", "S14", "S15", "S16", "S17", "S18", "S19", "S20", "S21", "S22", "S23", "S24", "S25", "S26", "S27", "S28", "S29", "S30", "S31", "S32", "S33", "S34", "S35", "S36", "S37", "S38", "S39", "S40"] + subject
    ...
    if __name__ == '__main__':
        ...
        for epoch in range(initial_epoch, args.epoch):
            ...
            for n_count, batch_tr in enumerate(TrainLoader):
                ...
                y_pred = model(batch_emg)  # BERT/TCN
            ...
            with torch.no_grad():
                ...
                for _, batch_eval in enumerate(DLoader_eval):
                    ...
                    glove_pred = model(batch_eval[0].squeeze(3).cuda() )  # BERT/TCN
