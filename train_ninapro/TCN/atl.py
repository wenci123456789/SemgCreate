# atl_tcn.py
# 仅用于校准 TCN 的 ATL 脚本
# - Backbone: sEMG_TCN(num_inputs=12, num_channels=[128,128,128,128,10], kernel_size=3, dropout=0.7)
# - ATL: 判别器只看“头前特征”（hook 到 fc 的输入），student 将目标特征混淆成“源”
# - 输出：TCN 通常返回 [B,1,10]，训练/评估统一展平为 [B,10]
...
class HookedModelTCN(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, head_name: str = "fc"):
        ...
        if head_name in named:
            ...
            if last_linear is None:
                raise RuntimeError("TCN 主干中未找到任何 nn.Linear 作为输出头。")
            target = last_linear
    ...
    def forward_with_features(self, x: torch.Tensor):
        out = self.backbone(x)          # 预期 [B,1,10]
        y = out[0] if isinstance(out, (tuple, list)) else out
        return y, self._feat

# ---------------- 主训练（单目标被试） ----------------
def run_atl_tcn_for_target(args, device, target_subject: str, source_subjects: List[str]):
    # 目标域/测试加载
    ...
    with torch.no_grad():
        ...
        xb = xb.squeeze(3).to(device).float()   # [B, T, C]，TCN 的 forward 里会处理
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_t in T_loader:
            x_t, y_t, *_ = batch_t
            x_t = x_t.squeeze(3).to(device).float()               # [B,T,C]
            ...
            y_hat, f_t = nt.forward_with_features(x_t)  # y_hat 预期 [B,1,10]
        ...
        print(f"[ATL-TCN {target_subject}] Epoch {epoch:03d}  "
...
# ---------------- 入口 ----------------
def main():
    ap = argparse.ArgumentParser(description="ATL calibrate — TCN backbone (NinaPro)")
    # data
    ...
    ap.add_argument('--save_dir', type=str, default='../../result/ninapro/Estimation_result/TCN/checkpoints_atl')
    ...
    for tgt in args.targets:
        print(f"\n====== ATL-TCN start: {tgt} ======")
        run_atl_tcn_for_target(args, device, tgt, args.source_subjects)
        print(f"====== ATL-TCN done : {tgt} ======\n")
