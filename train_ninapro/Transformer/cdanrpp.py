...
def build_loader(root, sid, subframe, normalization, mu, batch_size, shuffle, drop_last):
    emg_tr = os.path.join(root, f"{sid}_E2_A1_rms_train.h5")
    glo_tr = os.path.join(root, f"{sid}_E2_A1_glove_train.h5")
    emg_te = os.path.join(root, f"{sid}_E2_A1_rms_test.h5")
    glo_te = os.path.join(root, f"{sid}_E2_A1_glove_test.h5")

    ds_tr = NinaPro.NinaPro(
        emg_tr, glo_tr, subframe=subframe,
        normalization=normalization, mu=mu,
        dummy_label=0, class_num=1
    )
    ds_te = NinaPro.NinaPro(
        emg_te, glo_te, subframe=subframe,
        normalization=normalization, mu=mu,
        dummy_label=0, class_num=1
    )
...
def build_source_loader(root: str, subjects: List[str], subframe: int,
                        normalization: str, mu: float, batch_size: int):
    S_sets = []
...
# ---------------- Hook: 抓头前特征（Transformer） ----------------
class HookedModelTransformer(nn.Module):
    """
    - 优先找 'linear1'；找不到就回退为最后一个 nn.Linear
    - 钩“线性层的输入”（头前特征）
    """
    def __init__(self, backbone: nn.Module, head_name: str = "linear1"):
        ...
        if head_name in named:
            ...
            if last_linear is None:
                raise RuntimeError("Transformer 主干未找到 nn.Linear 作为输出头。")
            target = last_linear
            self.head_name = last_name
            print(f"[HookedModel-Transformer] 未找到 '{head_name}'，使用最后一个 Linear: '{last_name}'")
...
class CDANRDiscriminator(nn.Module):
    ...
    def forward(self, f: torch.Tensor, y_code: torch.Tensor) -> torch.Tensor:
        ...
        outer = torch.bmm(f.unsqueeze(2), y_code.unsqueeze(1))  # [B,D,1]×[B,1,C]→[B,D,C]
...
# ---------------- 训练（单目标） ----------------
def run_cdanr_transformer_for_target(args, device, target_subject: str, source_subjects: List[str]):
    # data
    T_loader, Te_loader = build_loader(
        args.data_root, target_subject,
    ...
    S_loader = build_source_loader(
        args.data_root, src_list,
        args.subframe, args.normalization, args.miu, args.batch_size
    )

    # teacher (frozen) / student (to calibrate) —— 与预训练 Transformer 同构
    ms_net = MAFN(
        winsize=200, patch_size=1, in_c=1,
        num_classes=10, depth=4, num_heads=4, embed_dim=12,
        attn_drop_ratio=0.0, drop_ratio=0.3
    ).to(device)
    ...
    # 仅训练输出相关参数（Transformer 的回归头 / adapter 等）
    ...
    with torch.no_grad():
        ...
        xb = xb.squeeze(3).to(device).float()   # [B,200,12]
        _y_try, f0 = nt.forward_with_features(xb)
        feat_dim = f0.size(-1) if f0.dim() >= 2 else int(np.prod(f0.shape[1:]))
    y_dim = 10

    D = CDANRDiscriminator(f_dim=feat_dim, y_dim=y_dim,
                           hidden=args.d_hidden, p=0.2).to(device)
    reg_loss = nn.MSELoss()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for (xb_t, yb_t, *_) in T_loader:
            ...
            xb_t = xb_t.squeeze(3).to(device).float()          # [B,200,12]
            ...
            for p in D.parameters():
                p.requires_grad = True
            nt.zero_grad(set_to_none=True)
            D.zero_grad(set_to_none=True)

            _, f_s = ms.forward_with_features(xb_s)
            y_hat_t_cur, f_t = nt.forward_with_features(xb_t)
            if isinstance(y_hat_t_cur, (tuple, list)):
                y_hat_t_cur = y_hat_t_cur[0]
            if y_hat_t_cur.dim() == 3:
                y_hat_t_cur = y_hat_t_cur.view(y_hat_t_cur.size(0), -1)  # safety
            ...
            if isinstance(y_hat_t, (tuple, list)):
                y_hat_t = y_hat_t[0]
            if y_hat_t.dim() == 3:
                y_hat_t = y_hat_t.view(y_hat_t.size(0), -1)  # safety
            ...
            L_adv = bce_logits(logit_s_g, torch.ones_like(logit_s_g)) + \
                    bce_logits(logit_t_g, torch.ones_like(logit_t_g))
        ...
        with torch.no_grad():
            ...
            try:
                ...
                CC_mean10 = float(np.mean(
                    [pearson_CC(y_np[:, i], yh_np[:, i]) for i in range(10)]
                ))
        ...
        print(
            f"[CDANR-Trans {target_subject}] Epoch {epoch:03d}  "
            f"LRg={cur_lr_g:.2e} LRd={cur_lr_d:.2e}  "
            f"train_mse={total_reg/len(T_loader):.6f}  "
            f"train_adv={total_adv/len(T_loader):.6f}  "
            f"train_D={total_d/len(T_loader):.6f}  "
            f"{'train_R1='+format(total_r1/len(T_loader),'.6f') if args.r1_gamma>0 else ''}  "
            f"Val(MSE)={val_mse:.6f}  NRMSE={NRMSE:.4f}  "
            f"CC(mean@10)={CC_mean10:.4f}  R2={R2:.4f}  "
            f"lam_adv={lam_adv:.3f}"
        )

        # save latest
        torch.save(
            {'epoch': epoch, 'model_state': nt_net.state_dict()},
        ...
        if args.select_metric == 'mse':
            cur_score = val_mse
            is_better = cur_score < best_score
                ...
                torch.save(
                    {
                        ...
                        'saved_with_ema': bool(ema is not None)
                    },
...
# ---------------- main ----------------
def main():
    ap = argparse.ArgumentParser(description="CDANR calibration — Transformer backbone (NinaPro)")
    # data
    ...
    ap.add_argument('--save_dir', type=str, default='../../result/ninapro/Estimation_result/Transformer/checkpoints_cdanrpp')
    ...
    for tgt in args.targets:
        print(f"\n====== CDANR-Transformer start: {tgt} ======")
        run_cdanr_transformer_for_target(args, device, tgt, args.source_subjects)
        print(f"====== CDANR-Transformer done : {tgt} ======\n")
