# cdanr_nocorr_notaa.py
# 说明：
# - 无 CorrBoost、无 TTA
# - best.pth 按“十个关节逐关节 Pearson CC 的平均值”保存
# - 仍保留对抗对齐（GRL+warm-up）与 R1（可通过 --r1_gamma 控制；消融时设 0）
# - 只微调 Adapter+Head，骨干冻结
...
def aug_channel_drop(x: torch.Tensor, p: float = 0.1) -> torch.Tensor:
    # x: [B,T,C]，按通道丢弃（同一 batch 的时间维共享掩码）
    if p <= 0: return x
    B, T, C = x.shape
...
class HookedModel(nn.Module):
    """
    捕获输出头（默认 'output_proj'）的输入作为“头前特征” f，并提供 apply_head(f)。
    """
...
def run_cdanrpp_for_target(args, device, target_subject: str, source_subjects: List[str]):
    ...
    # --- best 选择逻辑（按 select_metric），我们将 CC 定义为“逐关节 CC 的平均” ---
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for (xb_t, yb_t, *_) in T_loader:
            ...
            f_s_rg = f_s.detach().requires_grad_(args.r1_gamma > 0)  # 需要 R1 时才开 grad
            ...
            L_adv = bce_logits(logit_s_g, torch.ones_like(logit_s_g)) + bce_logits(logit_t_g, torch.zeros_like(logit_t_g))
        ...
        # best：按 args.select_metric，其中 CC 已定义为“逐关节均值”
        ...
        if is_better and not (isinstance(cur_score, float) and (math.isnan(cur_score) or math.isinf(cur_score))):
            ...
            try:
                torch.save(
                    {'epoch': epoch, 'model_state': nt_net.state_dict(), 'saved_with_ema': bool(ema is not None)},
                    os.path.join(save_dir_one, 'cdanrpp_best.pth'))
                # 也可同步导出整模型（如需）：关闭 hook 后保存
                # _clear_all_hooks(nt_net); torch.save(nt_net, os.path.join(save_dir_one, 'model_best.pth'))
            finally:
                if ema is not None:
                    ema.restore()
...
def main():
    ...
    # selection metric（改成默认按 CC；如需别的可命令行覆盖）
