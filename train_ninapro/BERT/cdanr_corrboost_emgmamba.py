...
def pearson_corr(y_hat, y, eps: float = 1e-8):
    ...
    y_hat = y_hat - y_hat.mean(dim=0, keepdim=True); y = y - y.mean(dim=0, keepdim=True)
    num = (y_hat * y).sum(dim=0); den = (y_hat.pow(2).sum(dim=0).sqrt() * y.pow(2).sum(dim=0).sqrt() + eps)
    return (num / den).mean()

def corrboost_loss(y_hat, y, eps: float = 1e-6):
    if y_hat.dim() == 3 and y_hat.size(1) == 1: y_hat = y_hat.squeeze(1)
    if y.dim() == 3 and y.size(1) == 1: y = y.squeeze(1)
    yh_c = y_hat - y_hat.mean(dim=0, keepdim=True); y_c = y - y.mean(dim=0, keepdim=True)
    num = (yh_c * y_c).sum(dim=0); den = (yh_c.pow(2).sum(dim=0).sqrt() * y_c.pow(2).sum(dim=0).sqrt() + eps)
    rho = num / den
    one_minus_rho = (1.0 - rho).mean()
    rho_c = rho.clamp(-1 + 1e-4, 1 - 1e-4)
    z = 0.5 * torch.log((1 + rho_c) / (1 - rho_c))
    return one_minus_rho + 0.25 * (-z).mean()

class GRL(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, lambd: float): ctx.lambd = lambd; return x.view_as(x)
    @staticmethod
    def backward(ctx, grad_output): return -ctx.lambd * grad_output, None
def grl(x, lambd: float): return GRL.apply(x, lambd)
...
def aug_channel_drop(x: torch.Tensor, p: float = 0.1) -> torch.Tensor:
    if p <= 0: return x
    B,T,C = x.shape; mask = (torch.rand(B, C, device=x.device) > p).float()
...
def build_loader(root, sid, subframe, normalization, mu, batch_size, shuffle, drop_last):
    ...
    ds_tr = NinaPro.NinaPro(e, g, subframe=subframe, normalization=normalization, mu=mu, dummy_label=0, class_num=1)
...
def build_source_loader(root, subjects: List[str], subframe, normalization, mu, batch_size):
    S_sets = []
    for i, sid in enumerate(subjects):
        e = os.path.join(root, f"{sid}_E2_A1_rms_train.h5")
        g = os.path.join(root, f"{sid}_E2_A1_glove_train.h5")
        S_sets.append(NinaPro.NinaPro(e, g, subframe=subframe, normalization=normalization, mu=mu, dummy_label=i, class_num=len(subjects)))
    return DataLoader(ConcatDataset(S_sets), batch_size=batch_size, shuffle=True, drop_last=True, num_workers=0, pin_memory=False)
...
def run_cdanrpp_for_target(args, device, target_subject: str, source_subjects: List[str]):
    ...
    def build_warmup_cosine_scheduler(optimizer, total_epochs, warmup_epochs, base_lr, final_lr_ratio=0.1):
        import math as _m
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        nt.train(); D.train()
        total_reg=total_corr=total_adv=total_d=total_r1=0.0
        ...
        for (xb_t, yb_t, *_) in T_loader:
            ...
            yb_t = yb_t.to(device).float().view(yb_t.size(0), -1)
            ...
            total_reg += float(L_mse.item()); total_corr += float(L_corr.item()); total_adv += float(L_adv.item())
...
def main():
    ...
    ap.add_argument('--save_dir', type=str, default='../../result/ninapro/Estimation_result/BERT/checkpoints_cdanrpp')
