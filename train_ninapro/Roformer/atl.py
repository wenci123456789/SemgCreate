# atl_roformer.py
# 仅针对 RoFormer 的 ATL 校准脚本（对齐 BERT-ATL 流程）
# - Backbone: RoFormerEMG(12->10, d_model=120, num_layers=2, num_heads=5, use_mu_law=False)
# - ATL: 判别器只看“头前特征”，目标特征被混淆为“源”
# - RoFormer 输出可能为 [B,T,10]，训练/评估时对 T 取均值池化到 [B,10]
...
class HookedModel(nn.Module):
    """
    RoFormer 专用 Hook：
    - 优先找 'regressor'；找不到就回退为模型里最后一个 nn.Linear
    - 钩的是该线性层的“输入张量”，即头前特征（可能是 [B,T,D]）
    """
    def __init__(self, backbone: nn.Module, head_name: str = "regressor"):
        super().__init__()
...
# ---------------- 主训练（单目标被试） ----------------
def run_atl_roformer_for_target(args, device, target_subject: str, source_subjects: List[str]):
    # 目标域/测试加载
    ...
    ms_net.load_state_flex = load_state_flex  # 仅为统一调用
    ...
    # 只训练输出相关参数（没有 adapter 就训练 regressor/最后线性层）
    ...
    # 日志与保存
    save_dir_one = os.path.join(args.save_dir, f"atl_roformer_{target_subject}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_t in T_loader:
            x_t = batch_t[0].squeeze(3).to(device).float()      # [B,T,C]
            ...
            y_hat, f_t = nt.forward_with_features(x_t)     # y_hat 可能是 [B,T,10]
        ...
        print(f"[ATL-RoFormer {target_subject}] Epoch {epoch:03d}  "
...
# ---------------- 入口 ----------------
def main():
    ap = argparse.ArgumentParser(description="ATL calibrate — RoFormer backbone (NinaPro)")
    # data
    ...
    ap.add_argument('--save_dir', type=str, default='../../result/ninapro/Estimation_result/Roformer/checkpoints_atl')
    ...
    for tgt in args.targets:
        print(f"\n====== ATL-RoFormer start: {tgt} ======")
        run_atl_roformer_for_target(args, device, tgt, args.source_subjects)
        print(f"====== ATL-RoFormer done : {tgt} ======\n")
