...
- 每一折：
    * 训练集：本折以外的被试，仅使用 *_rms_train.h5
    * 验证集：本折内的被试，仅使用 *_rms_test.h5
- 模型：sEMG_BERT
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Pretrain BERT Multi-s-net with subject-level K-fold')
    # 默认和 Mamba 一样的 15 个被试；按需改成 S1~S30
    ...
    parser.add_argument('--save_dir', type=str,
                        default='../result/ninapro/checkpoints_fold_pretrain/BERT')
    ...
    for fold_idx, val_subjects_np in enumerate(folds):
        ...
        print("\n" + "=" * 60)
        print(f"⭐ BERT Fold {fold_idx+1}/{args.k_folds}")
        ...
        for epoch in range(1, args.epoch + 1):
            ...
            print(f"[BERT Fold {fold_idx+1} | Epoch {epoch:03d}] "
        ...
        print(f"[BERT Fold{fold_idx+1}] ✅ Finished. Best NRMSE: {best_nrmse:.4f}")

    print("🎉 BERT K-fold pretraining finished.")
